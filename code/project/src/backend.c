...
void swindow_send(cmu_socket_t *sock, char *data, int buf_len, uint32_t seq_from);
...
void send_packet_with_seq_ack_flags(cmu_socket_t *sock, int seq, int ack, uint8_t flags);
...
void send_packet_with_seq_ack_flags(cmu_socket_t *sock, int seq, int ack, uint8_t flags) {
  ...
  printf("[DEBUG] [send_packet_with_seq_ack_flags] send flags packet, seq=%d, ack=%d, flags=%d\n", seq, ack, flags);
  while (TRUE) {
    sendto(sock->socket, pkt, DEFAULT_HEADER_LEN, 0, (struct sockaddr *) &(sock->conn), conn_len);
    check_for_data_inner(sock, TIMEOUT);
    if (check_ack(sock, seq)) {
      if (!(flags & SYN_FLAG_MASK) || sock->tcp_state == ESTABLISHED)
        break;
    }
  }
  free(pkt);
...
void update_timer(cmu_socket_t *sock) {
    ...
    sock->samp_rtt = (sock->recv_time.tv_sec - sock->send_time.tv_sec) * 1000000 +
                     (sock->recv_time.tv_usec - sock->send_time.tv_usec);
 ...
 * 
 * Purpose: build an ack packet.
*/
char* build_ack(cmu_socket_t *sock, uint32_t ack) {
    char *pkt;
    uint32_t adv_window = get_adv_window(sock);

    printf("[DEBUG] [build_ack] ack=%d\n", ack);
...
void handle_message(cmu_socket_t *sock, char *pkt) {
    ...
    switch (flags) {
        case FIN_FLAG_MASK:
            // 收到 fin向对面发送ack
            if (sock->tcp_state == FIN_SENT) {
                sock->tcp_state = CLOSN_WAIT;

                // send ack
                rsp = build_ack(sock, seq + 1);
                sendto(sock->socket, rsp, DEFAULT_HEADER_LEN, 0, (struct sockaddr *) &(sock->conn), sizeof(sock->conn));
                free(rsp);
            }
            return;
...
//  sendto(sock->socket, pkt, DEFAULT_HEADER_LEN, 0, (struct sockaddr*) &(sock->conn), conn_len);
            ...
            rsp = create_packet_buf(sock->my_port, ntohs(sock->conn.sin_port), CLIENT_ISN + 1, seq + 1,
                                    DEFAULT_HEADER_LEN, DEFAULT_HEADER_LEN, ACK_FLAG_MASK, 1, 0, NULL, NULL, 0);

            sendto(sock->socket, rsp, DEFAULT_HEADER_LEN, 0, (struct sockaddr *) &(sock->conn), sizeof(sock->conn));
            free(rsp);
            printf("[DEBUG] [handle_message] [client] hand shake complete client\n");
            return;
        case ACK_FLAG_MASK:
            // 服务端收到客户端的ack 服务端结束
            if (sock->tcp_state == SYN_RCVD) {
                sock->tcp_state = ESTABLISHED;
                printf("[DEBUG] [handle_message] [server] hand shake complete server\n");
            }
        ...
        // send duplicated ack and expected seq when recieve unexpected seq
            if (seq != sock->window.expected_rev_seq && sock->window.expected_rev_seq > 0) {
                ...
                rsp = build_ack(sock, sock->window.expected_rev_seq);
                sendto(sock->socket, rsp, DEFAULT_HEADER_LEN, 0, (struct sockaddr *) &(sock->conn), sizeof(sock->conn));
                free(rsp);
                return;
            }
...
void swindow_send(cmu_socket_t *sock, char *data, int buf_len, uint32_t seq_from) {
    ...
    printf("[DEBUG] [swindow_send] send data, len=%d, seq=%d\n", buf_len, seq);
    if (buf_len > 0) {
        while (buf_len != 0) {
            ...
            if (buf_len <= MAX_DLEN) {
                ...
                msg = create_packet_buf(sock->my_port, sock->their_port, seq, ack,
                                        DEFAULT_HEADER_LEN, plen, flags, adv_window, 0, NULL, data + sent_len,
                                        MAX_DLEN);
                buf_len = buf_len - MAX_DLEN;
                seq = seq + MAX_DLEN;
            }
            printf("[DEBUG] [swindow_send] send packet, plen=%d, seq=%d\n", plen, get_seq(msg));
...
void resend(cmu_socket_t *sock) {
    ...
    swindow_send(sock, sock->tmp_buf + sent_size, send_len, send_base);
}
...
void begin_send(cmu_socket_t *sock) {
    ...
    while (TRUE) {
        ...
        printf("[DEBUG] [begin_send] start swindow_send\n");
        swindow_send(sock, sock->tmp_buf, sock->tmp_len, sock->window.last_ack_received);
        while (TRUE) {
...
void *begin_backend(void *in) {
    ...
    while (TRUE) {
        ...
        if (death && buf_len == 0 && dst->tcp_state == CLOSN_WAIT) {
        ...
        if (buf_len == 0 && dst->tmp_len == 0 && dst->dying) {
            pthread_cond_signal(&(dst->terminate_cond));
        }
